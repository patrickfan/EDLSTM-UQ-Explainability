...
class CL_plotter:
    ...
    def plotLSTM_raw_Y(self, yTrain, yValid, yTest, train_idx, valid_idx, scalery=None, figname=None, savefig=None, ylim_1=None, ylim_2=None):
        ...
        yTest_flat = yTest.reshape(yTest.shape[0]*yTest.shape[1])
        ...
        ax1.scatter(train_idx, yTrain[:,-1], color='g', s=2, alpha=0.5, label='Train Y')
        ax1.scatter(valid_idx, yValid[:,-1], color='b', s=2, alpha=0.5, label='Valid Y')

        ax1.set_title('Train/Valid')
        ax1.set_xlabel('Time (d)')
        ax1.set_ylabel('Discharge at {} (m^2/s)'.format(self.configs['data_name']))

        ax2.scatter(np.arange(len(yTest)*yTest.shape[1]), yTest_flat, color='r', s=20, alpha=0.5, label='Test Y')
        ax2.set_title('Test')
        ...
        if savefig is not None:
            plt.savefig(savefig, bbox_inches='tight',pad_inches=0.1, dpi=300)
        # plt.show()
        plt.close()

    ### plot LSTM encoder predictions
    def plotLSTM_encoder(self, num_outputs, train_idx, valid_idx, train_results_np, valid_results_np, test_results_np, ori_yTrain, ori_yValid, ori_yTest,figname=None, savefig=None, plot_hline=None, ylim_1=None, ylim_2=None, show_plot=False):
        ...
        for i in range(num_outputs):
            trainValid_pred_np[train_idx, i] = train_results_np[:, i]
            trainValid_pred_np[valid_idx, i] = valid_results_np[:, i]
            trainValid_ori_np[train_idx, i] = ori_yTrain[:, i]
            trainValid_ori_np[valid_idx, i] = ori_yValid[:, i]
        r2_trainValid = r2_score(trainValid_ori_np,trainValid_pred_np)
        r2_test = r2_score(ori_yTest,test_results_np)

        fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, figsize=(12,10))

        ax1.scatter(train_idx, ori_yTrain[:,-1], marker='o', color='g', s=20, alpha=0.3, label='Obs Train Y')
        ax1.scatter(train_idx, train_results_np[:,-1], marker='o', color='b', s=20, alpha=0.3, label='Pred Train Y')

        ax1.scatter(valid_idx, ori_yValid[:,-1], marker='*', color='g', s=20, alpha=0.3, label='Obs Valid Y')
        ...
        ori_yTest_flat = ori_yTest.reshape(ori_yTest.shape[0]*ori_yTest.shape[1])
        pre_yTest_flat = test_results_np.reshape(test_results_np.shape[0]*test_results_np.shape[1])
        ax2.plot(ori_yTest_flat,'r', label='Observation')
        ax2.plot(pre_yTest_flat,'b--',label='Test prediction')
    ...
    def plot_R2_evaluation(self, trainR2_mean, validR2_mean,testR2_mean, savefig=None):
        plt.plot(trainR2_mean,'b',label='Train')
        plt.plot(validR2_mean,'g',label='Valid')
        plt.plot(testR2_mean, 'r', label='Test')
        plt.scatter(range(len(trainR2_mean)), trainR2_mean, marker='o', color='b', s=20, alpha=0.3)
        plt.scatter(range(len(validR2_mean)), validR2_mean, marker='o', color='g', s=20, alpha=0.3)
        plt.scatter(range(len(testR2_mean)), testR2_mean, marker='o', color='r', s=20, alpha=0.3)
        plt.xlabel('Epochs')
        plt.ylabel('R2')
        plt.legend()
        plt.savefig(savefig , bbox_inches='tight',pad_inches=0.1, dpi=300)
