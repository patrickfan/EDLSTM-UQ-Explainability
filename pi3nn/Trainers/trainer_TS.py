...
class CL_trainer_TS:
    ...
    def train_LSTM_encoder(self):
        ...
        if self.configs['batch_training'] == True:
            ...
            for i in range(self.configs['Max_lstm_iter']):
                ...
                ### lr decay
                if self.configs['exponential_decay']:  #not familiar
                ...
                if self.allTestDataEvaluationDuringTrain:
                    ...
                    train_predictions = tf.reshape(train_predictions, (train_predictions.shape[0], train_predictions.shape[1]))
                    valid_predictions = tf.reshape(valid_predictions, (valid_predictions.shape[0], valid_predictions.shape[1]))
                    test_predictions = tf.reshape(test_predictions, (test_predictions.shape[0], test_predictions.shape[1]))
                    ...
                    yTrain = self.yTrain.reshape((self.yTrain.shape[0], self.yTrain.shape[1]))
                    yValid = self.yValid.reshape((self.yValid.shape[0], self.yValid.shape[1]))
                    yTest = self.yTest.reshape((self.yTest.shape[0], self.yTest.shape[1]))

                    yTrain = self.scalery.inverse_transform(yTrain)
                    yValid = self.scalery.inverse_transform(yValid)
                    yTest = self.scalery.inverse_transform(yTest)
                ...
                if stop_training:
                    ...
                    if not self.testDataEvaluationDuringTrain:
                        ...
                        break 
        self.plotter.plot_R2_evaluation(self.trainR2_mean, self.validR2_mean,self.testR2_mean, savefig=self.configs['project']+self.configs['exp']+'/'+self.configs['save_encoder_results']+'/'+'R2.png')
    ...
    def LSTMEncoderEvaluation(self, scalerx, scalery, save_results=False, return_results=False):
        ...
        train_results_np = yTrain_pred
        valid_results_np = yValid_pred
        test_results_np = yTest_pred
        ...
        for i in range (yTrain.shape[1]):
            nse_train = r2_score(yTrain[:,i], yTrain_pred[:,i])
            nse_valid = r2_score(yValid[:,i], yValid_pred[:,i])
            nse_test  = r2_score(yTest[:,i],  yTest_pred[:,i])
            print ("the day {} test score {}".format(i, nse_test))
            train_scores.append(nse_train)
            valid_scores.append(nse_valid)
            test_scores.append(nse_test)
        print ("The total test score {}".format(r2_score(yTest, yTest_pred)))
        if return_results:
            return r2_train, r2_valid, r2_test, train_scores, valid_scores, test_scores, train_results_np, valid_results_np, test_results_np
        else:   
