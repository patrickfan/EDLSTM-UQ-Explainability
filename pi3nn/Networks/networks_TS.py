...
class LSTM_encoder_decoder (Model):
	def __init__ (self, configs, num_outputs):
		...
		self.configs = configs
		self.x_lstm1 = LSTM (self.configs['LSTM_nodes'],activation='relu', return_sequences=False, name='lstm1')
		self.x_repvc = RepeatVector(num_outputs)
		self.x_lstm2 = LSTM (self.configs['LSTM_nodes'], activation='relu', return_sequences=True, name='lstm2')
		self.lstm_mean = TimeDistributed(Dense(self.configs['LSTM_nodes']/2, activation='relu', name='lstm_mean')) 
		self.outputLayer = TimeDistributed(Dense(1, name='dense_out'))
